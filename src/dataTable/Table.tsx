...
export default function Table<T extends RowType>(props: TableProps<T>): JSX.Element {
  const { columns, rows, selectable, selectAll, loading, 
        onRowClick, onSelectChange, onSelectAllChange, onScrollToEnd } = props;
      ...
      <thead>
        <tr>
          ...
          {columns.map((column, index) => {
            return (
              <th style={{ width: column.width }} key={`col-${index}`}>
                {column.label}
              </th>
            );
          })}
      ...
      <tbody>
        {rows.map((row, index) => {
          ...
          return (
            <tr
              key={`Row-${index}`}
              ref={index + 1 === rows.length ? lastItemRef : undefined}
              onClick={() => onRowClick?.(row, index)}
            >
              {selectable && (
                <td key={`Row-${index}-Col-CheckBox`}>
                  <input
                    type={"checkbox"}
              ...
              {columns.map((column, colIndex) => {
                return (
                  <td
                    className={column.numeric ? "cell-right-aligned" : ""}
